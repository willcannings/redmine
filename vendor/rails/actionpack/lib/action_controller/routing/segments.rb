  ...
  module Routing
    ...
    class DynamicSegment < Segment #:nodoc:
      ...
      def initialize(key = nil, options = {})
        ...
        # custom voodoo to make id's and project id's work with full stops. the default
        # regexp for dynamic segments was changed when the :format option was added,
        # making it ([^/ . ?]+). this if resets it to ([^/?]+) for :id and :project_id
        # where a custom regular expression hasn't previously been set
        if !options.key?(:regexp) && (key == :id || key == :project_id)
          @regexp = /([^\/\?]+)/
        end
      end
